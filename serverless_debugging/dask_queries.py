...
def query_1(dataset_path, fs, scale):
    ...
    lineitem_ds = dd.read_parquet(
        dataset_path + "lineitem", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    ...
    lineitem_filtered["sum_disc_price"] = lineitem_filtered.l_extendedprice * (1 - lineitem_filtered.l_discount)
...
def query_2(dataset_path, fs, scale):
    ...
    region_ds = dd.read_parquet(
        dataset_path + "region", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    nation_filtered = dd.read_parquet(
        dataset_path + "nation", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    supplier_filtered = dd.read_parquet(
        dataset_path + "supplier", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    part_filtered = dd.read_parquet(
        dataset_path + "part", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    partsupp_filtered = dd.read_parquet(
        dataset_path + "partsupp", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
...
def query_3(dataset_path, fs, scale):
    ...
    orders_ds = dd.read_parquet(
        dataset_path + "orders", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    cutomer_ds = dd.read_parquet(
        dataset_path + "customer", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
...
def query_4(dataset_path, fs, scale):
    ...
    forders = orders_ds[osel]
    jn = forders.merge(flineitem, how="leftsemi", left_on="o_orderkey", right_on="l_orderkey")
    result_df = (
        jn.groupby("o_orderpriority").size().to_frame("order_count").reset_index().sort_values(["o_orderpriority"])
    )
    return result_df
...
def query_5(dataset_path, fs, scale):
    ...
    nation_ds = dd.read_parquet(
        dataset_path + "nation", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    customer_ds = dd.read_parquet(
        dataset_path + "customer", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    line_item_ds = dd.read_parquet(
        dataset_path + "lineitem", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
...
def query_7(dataset_path, fs, scale):
    ...
    lineitem_filtered = line_item_ds[(line_item_ds["l_shipdate"] >= var1) & (line_item_ds["l_shipdate"] < var2)]
...
def query_8(dataset_path, fs, scale):
    ...
    supplier = dd.read_parquet(
        dataset_path + "supplier", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    lineitem = dd.read_parquet(
        dataset_path + "lineitem", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    orders = dd.read_parquet(
        dataset_path + "orders", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    customer = dd.read_parquet(
        dataset_path + "customer", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    nation = dd.read_parquet(
        dataset_path + "nation", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
    region = dd.read_parquet(
        dataset_path + "region", filesystem=fs, engine=ENGINE, chunksize=CHUNK_SIZE, aggregate_files=AGGREGATE_FILES
    )
...
def query_9(dataset_path, fs, scale):
    ...
    subquery["amount"] = (
        subquery.l_extendedprice * (1 - subquery.l_discount) - subquery.ps_supplycost * subquery.l_quantity
    )
...
def query_10(dataset_path, fs, scale):
    ...
    orders = orders[(orders.o_orderdate >= orderdate_from) & (orders.o_orderdate < orderdate_to)]
...
def query_11(dataset_path, fs, scale):
    ...
    joined = partsupp.merge(supplier, left_on="ps_suppkey", right_on="s_suppkey", how="inner").merge(
    ...
    res = res[res > threshold].round(2).reset_index().sort_values(by="value", ascending=False)
...
def query_14(dataset_path, fs, scale):
    ...
    total_revenue = (table.l_extendedprice * (1 - table.l_discount)).to_frame().sum().reset_index(drop=True)
    # aggregate promo revenue calculation
    return (100.00 * total_promo_revenue / total_revenue).round(2).to_frame("promo_revenue")
...
def query_15(dataset_path, fs, scale):
    ...
    # Create revenue view
    lineitem = lineitem[(lineitem.l_shipdate >= shipdate_from) & (lineitem.l_shipdate < shipdate_to)]
    ...
    # Query
    table = supplier.merge(revenue, left_on="s_suppkey", right_on="supplier_no", how="inner")
...
def query_16(dataset_path, fs, scale):
    ...
    complaint_suppkeys = supplier[supplier.is_complaint].s_suppkey.repartition(npartitions=1)
...
def query_17(dataset_path, fs, scale):
    ...
    avg_qnty_by_partkey = (
        joined.groupby("l_partkey").l_quantity.mean().to_frame().rename(columns={"l_quantity": "l_quantity_avg"})
    )
...
def query_18(dataset_path, fs, scale):
    ...
    qnt_over_300 = lineitem.groupby("l_orderkey").l_quantity.sum(split_out=True).reset_index()
    qnt_over_300 = qnt_over_300[qnt_over_300.l_quantity > 300]

    table = (
        orders.merge(qnt_over_300, left_on="o_orderkey", right_on="l_orderkey", how="leftsemi")
        .merge(lineitem, left_on="o_orderkey", right_on="l_orderkey", how="inner")
        .merge(customer, left_on="o_custkey", right_on="c_custkey", how="inner")
    )

    return (
        table.groupby(["c_name", "c_custkey", "o_orderkey", "o_orderdate", "o_totalprice"])
        .l_quantity.sum()
        .reset_index()
...
def query_19(dataset_path, fs, scale):
    ...
    return (table.l_extendedprice * (1 - table.l_discount)).to_frame().sum().round(2).to_frame("revenue")
...
def query_20(dataset_path, fs, scale):
    ...
    res_1 = lineitem[(lineitem["l_shipdate"] >= shipdate_from) & (lineitem["l_shipdate"] < shipdate_to)]
    res_1 = res_1.groupby(["l_suppkey", "l_partkey"])["l_quantity"].sum().rename("sum_quantity").reset_index()
    ...
    q_final = partsupp.merge(res_4, how="leftsemi", left_on="ps_partkey", right_on="p_partkey").merge(
        res_1,
...
def query_21(dataset_path, fs, scale):
    ...
    res_1 = lineitem.groupby("l_orderkey")["l_suppkey"].nunique().rename("nunique_col").reset_index()
    ...
    predicate = (q_final["nunique_col"] == 1) & (q_final["n_name"] == NATION) & (q_final["o_orderstatus"] == "F")
...
def query_22(dataset_path, fs, scale):
    ...
    customers = customers[customers["cntrycode"].isin(("13", "31", "23", "29", "30", "18", "17"))]
    ...
    custsale = custsale.merge(orders_ds, left_on="c_custkey", right_on="o_custkey", how="left")
