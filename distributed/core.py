...
class Server:
    ...
    *  ``{'op': 'add', 'x': 10, 'y': 20}``
    ...
    def identity(self, **kwargs) -> dict[str, str]:
...
class PooledRPCCall:
    ...
    def __init__(self, addr, pool, serializers=None, deserializers=None, extra_kwargs=None):
        self.addr = addr
        self.pool = pool
        self.serializers = serializers
        self.deserializers = deserializers if deserializers is not None else serializers
...
class ConnectionPool:
    ...
    def __call__(
        ...
        return PooledRPCCall(
            addr, self, serializers=self.serializers, deserializers=self.deserializers, extra_kwargs=self.extra_kwargs
        )
